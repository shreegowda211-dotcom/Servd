...
export default function PantryPage() {
  ...
  const startEdit = (item) => {
    ...
    if (!id) {
      toast.error("Unable to edit item: missing ID");
      return;
    }
    setEditingId(id);
    setEditValues({
  ...
  const saveEdit = async () => {
    if (!editingId) {
      toast.error("Unable to save changes: missing item ID");
      return;
    }
  ...
  const formatItemDate = (item) => {
    const dateValue =
      ...
      item.data?.attributes?.createdAt ||
      item.data?.attributes?.updatedAt ||
      item.data?.attributes?.created_at ||
      item.data?.attributes?.updated_at;
  ...
  const getItemName = (item) => item?.name || "Unnamed ingredient";

  const getItemQuantity = (item) => item?.quantity || "Unknown quantity";
