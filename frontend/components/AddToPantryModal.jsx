...
export default function AddToPantryModal({ isOpen, onClose, onSuccess }) {
  ...
  const handleSaveScanned = async () => {
    ...
    try {
      ...
      if (!result?.success) {
        toast.error(result?.message || "Unable to save scanned ingredients");
        return;
      }
    } catch (error) {
      // Error is already handled by useFetch toast
    }
  };
  ...
  const handleAddManual = async (e) => {
    ...
    try {
      ...
      if (!result?.success) {
        toast.error(result?.message || "Unable to add item to pantry");
      }
    } catch (error) {
      // Error is already handled by useFetch toast
    }
